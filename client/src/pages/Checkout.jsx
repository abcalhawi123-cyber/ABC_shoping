...
export default function Checkout() {
  ...
  useEffect(() => {
    getShippingZones().then(({ data }) => setZones(data.data));
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if (paymentMethod === 'instapay' && !form.instapayTransactionId) {
      return toast.error('أدخل كود العملية');
    }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 32, alignItems: 'start' }}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={sectionStyle}>
            <h3 style={sectionTitle}>📋 بيانات التواصل</h3>
          ...
          <div style={sectionStyle}>
            <h3 style={sectionTitle}>🚚 عنوان الشحن</h3>
            <div style={{ marginBottom: 16 }}>
              ...
              >
                ...
                {zones.map(z => (
                  <option key={z._id} value={z.governorate.ar}>{z.governorate.ar}</option>
                ))}
              </select>
              {selectedZone && (
                <p style={{ fontSize: 13, color: '#27ae60', marginTop: 6 }}>
                  🚚 رسوم الشحن: {selectedZone.price} ج.م | التوصيل: {selectedZone.estimatedDays.min}–{selectedZone.estimatedDays.max} أيام
                </p>
              )}
            </div>
            {field('city', t('city'))}
          ...
          <div style={sectionStyle}>
            ...
            {paymentMethod === 'instapay' && (
              <div style={{ background: '#f0f8ff', borderRadius: 10, padding: 16, marginTop: 8 }}>
                <p style={{ margin: '0 0 12px', fontSize: 14, color: '#1a3a5c' }}>
                  📲 حول المبلغ <strong>{total.toFixed(0)} ج.م</strong> على InstaPay
                </p>
                {field('instapayTransactionId', t('transactionId'))}
                <div>
                  <label style={labelStyle}>{t('uploadScreenshot')}</label>
                  <input
                    type="file"
                    accept="image/*"
            ...
            {paymentMethod === 'card' && (
              <div style={{ background: '#f0f8ff', borderRadius: 10, padding: 16, marginTop: 8 }}>
                <p style={{ fontSize: 14, color: '#555', margin: 0 }}>
                  ستنتقل إلى بوابة Paymob الآمنة لإتمام الدفع بعد تأكيد الطلب.
                </p>
              </div>
            )}
          </div>
          ...
          <button
            ...
            style={{
              ...
              background: submitting ? '#aaa' : '#1a3a5c',
        ...
        <div style={{ position: 'sticky', top: 90 }}>
          <div style={sectionStyle}>
            <h3 style={sectionTitle}>🛒 ملخص الطلب</h3>
            {cart.map(item => {
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span>{t('subtotal')}</span><span>{subtotal.toFixed(0)} ج.م</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
              <span>{t('shippingFee')}</span>
              <span>{selectedZone ? `${selectedZone.price} ج.م` : '—'}</span>
            </div>
...
const sectionStyle = { background: '#fff', borderRadius: 12, padding: '20px', marginBottom: 20, boxShadow: '0 1px 8px rgba(0,0,0,0.06)', border: '1px solid #f0e8e8' };
const sectionTitle = { margin: '0 0 16px', color: '#1a3a5c', fontSize: 17 };
const inputStyle = { width: '100%', padding: '11px 12px', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, boxSizing: 'border-box', background: '#fafafa' };
