...
export default function OrderTrack() {
  ...
  return (
    <div style={{ maxWidth: 580, margin: '0 auto', padding: '20px 16px' }}>
      <h1 style={{ color: '#6D1A36', marginBottom: 6, fontSize: 22 }}>📦 {t('trackOrder')}</h1>
      <p style={{ color: '#888', marginBottom: 24, fontSize: 13 }}>
        {lang === 'ar' ? 'رقم الطلب:' : 'Order ID:'} <span style={{ fontFamily: 'monospace', color: '#6D1A36', fontWeight: 600 }}>{id?.slice(-8).toUpperCase()}</span>
      </p>
        ...
        <>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 32, position: 'relative' }}>
            <div style={{ position: 'absolute', top: 15, right: '7%', left: '7%', height: 3, background: '#f0f2f5', zIndex: 0 }}>
              <div style={{ width: `${Math.max(0, step / (STEPS_AR.length - 1)) * 100}%`, height: '100%', background: '#6D1A36', transition: 'width 0.6s ease' }} />
            </div>
            {STEPS_AR.map((s, i) => (
              <div key={s} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 1, flex: 1 }}>
                <div style={{
                  width: 30, height: 30, borderRadius: '50%',
                  background: i <= step ? '#6D1A36' : '#e0e0e0',
                ...
                <span style={{ fontSize: 'clamp(9px,2vw,11px)', color: i <= step ? '#6D1A36' : '#aaa', fontWeight: i === step ? 700 : 400, textAlign: 'center', maxWidth: 64 }}>
                  {DISPLAY[i]}
                </span>
              </div>
            ))}
          ...
          <div style={{ background: '#fff', borderRadius: 12, padding: '16px', boxShadow: '0 2px 10px rgba(0,0,0,0.06)', marginBottom: 14 }}>
            <h3 style={{ color: '#6D1A36', margin: '0 0 12px', fontSize: 15 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700, fontSize: 16, color: '#6D1A36', marginTop: 8 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: '16px', boxShadow: '0 2px 10px rgba(0,0,0,0.06)' }}>
              ...
              {order.statusHistory.slice().reverse().map((h, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 8, flexWrap: 'wrap', gap: 4 }}>
                  <span style={{ fontWeight: 600, color: '#6D1A36' }}>{h.status}</span>
