...
export default function ReviewSection({ productId }) {
  ...
  return (
    <div style={{ marginTop: 48 }}>
      <h2 style={{ color: '#1a3a5c', borderRight: '4px solid #f8ad9d', paddingRight: 12, marginBottom: 20 }}>
      ...
      {reviews.map(r => (
        <div key={r._id} style={{ background: '#fff', borderRadius: 10, padding: '14px 18px', marginBottom: 12, border: '1px solid #f0e8e8', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, flexWrap: 'wrap', gap: 8 }}>
            <strong style={{ color: '#1a3a5c' }}>{r.user?.name || r.guestName || (lang === 'ar' ? 'زائر' : 'Guest')}</strong>
            <div>
      ...
      {hasMore && (
        <button
          ...
          disabled={loading}
          style={{ display: 'block', margin: '12px auto 24px', padding: '9px 28px', background: 'transparent', border: '2px solid #1a3a5c', color: '#1a3a5c', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 14 }}
        >
      ...
      <div style={{ background: '#faf5f5', borderRadius: 12, padding: 24, marginTop: 24, border: '1px solid #f0e8e8' }}>
        <h3 style={{ color: '#1a3a5c', margin: '0 0 18px', fontSize: 17 }}>{t('writeReview')}</h3>
        <form onSubmit={submit}>
          ...
          <button type="submit" disabled={submitting} style={{ background: '#1a3a5c', color: '#fff', border: 'none', borderRadius: 8, padding: '11px 28px', fontWeight: 600, cursor: submitting ? 'not-allowed' : 'pointer', fontSize: 14 }}>
