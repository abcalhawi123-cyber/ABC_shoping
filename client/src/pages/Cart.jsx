      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 28, alignItems: 'start' }}>
            ...
            return (
              <div key={item._id} style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ margin: 0, color: '#888', fontSize: 13 }}>{item.category?.[lang] || item.category?.ar}</p>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <button onClick={() => dispatch({ type: 'UPDATE_QTY', id: item._id, qty: item.qty - 1 })} style={qtyBtn}>−</button>
                  <span style={{ fontWeight: 700, minWidth: 28, textAlign: 'center' }}>{item.qty}</span>
                  <button onClick={() => dispatch({ type: 'UPDATE_QTY', id: item._id, qty: item.qty + 1 })} style={qtyBtn}>+</button>
                </div>
                <div style={{ textAlign: 'left', minWidth: 90 }}>
                  <p style={{ margin: 0, fontWeight: 700, fontSize: 16, color: '#1a3a5c' }}>{(price * item.qty).toFixed(0)} ج.م</p>
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.08)', border: '1px solid #f0e8e8', position: 'sticky', top: 90 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, color: '#888', fontSize: 14 }}>
            <span>{t('shippingFee')}</span><span>تُحدد عند الشراء</span>
          </div>
...
const qtyBtn = { width: 32, height: 32, border: '1px solid #ddd', borderRadius: 6, background: '#f5f5f5', cursor: 'pointer', fontSize: 16, fontWeight: 700 };
