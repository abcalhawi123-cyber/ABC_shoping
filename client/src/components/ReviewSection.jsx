...
export default function ReviewSection({ productId }) {
  ...
  return (
    <div style={{ marginTop: 40 }}>
      ...
      {reviews.map(review => (
        <div key={review._id} style={{ background: '#fff', borderRadius: 10, padding: '14px 18px', marginBottom: 12, boxShadow: '0 1px 6px rgba(0,0,0,0.06)', border: '1px solid #f0e8e8' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <strong style={{ color: '#1a3a5c' }}>{review.user?.name || review.guestName || 'زائر'}</strong>
      ...
      {hasMore && (
        <button onClick={() => fetchReviews(page + 1, true)} disabled={loading} style={{ display: 'block', margin: '12px auto 24px', padding: '9px 24px', background: 'transparent', border: '2px solid #1a3a5c', color: '#1a3a5c', borderRadius: 8, cursor: 'pointer', fontWeight: 600 }}>
      ...
      <div style={{ background: '#faf5f5', borderRadius: 12, padding: '20px', marginTop: 24 }}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 12 }}>
            <label style={ls}>اسمك</label>
            <input value={form.guestName} onChange={e => setForm({ ...form, guestName: e.target.value })} placeholder="اسمك (اختياري)" style={is} />
          </div>
          <div style={{ marginBottom: 12 }}>
            <label style={ls}>التقييم</label>
            <select value={form.rating} onChange={e => setForm({ ...form, rating: Number(e.target.value) })} style={is}>
              {[5,4,3,2,1].map(n => <option key={n} value={n}>{n} ★</option>)}
            </select>
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={ls}>تعليقك</label>
            <textarea required value={form.comment} onChange={e => setForm({ ...form, comment: e.target.value })} rows={3} maxLength={500} style={{ ...is, resize: 'vertical' }} />
          </div>
          <button type="submit" disabled={submitting} style={{ background: '#1a3a5c', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 24px', fontWeight: 600, cursor: 'pointer' }}>
