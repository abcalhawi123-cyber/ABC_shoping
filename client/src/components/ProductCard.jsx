    ...
    <Link to={`/products/${product.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
      ...
      >
        <div style={{ position: 'relative', aspectRatio: '1', overflow: 'hidden', background: '#faf5f5' }}>
          <img src={product.images?.[0]?.url || '/placeholder.jpg'} alt={name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} loading="lazy" />
          {product.discount > 0 && (
            <span style={{ position: 'absolute', top: 10, right: 10, background: '#f8ad9d', color: '#1a3a5c', padding: '3px 8px', borderRadius: 12, fontSize: 12, fontWeight: 700 }}>-{product.discount}%</span>
          )}
          {product.stock < 1 && (
          ...
          {product.stock > 0 && product.stock < 5 && (
            <span style={{ position: 'absolute', bottom: 8, right: 8, background: '#e74c3c', color: '#fff', padding: '2px 7px', borderRadius: 10, fontSize: 11 }}>
              {product.stock} {lang === 'ar' ? 'قطع فقط!' : 'left!'}
            </span>
          )}
        </div>
        <div style={{ padding: '12px 14px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <span style={{ fontSize: 17, fontWeight: 700, color: '#1a3a5c' }}>{effectivePrice.toFixed(0)} ج.م</span>
            {product.discount > 0 && <span style={{ fontSize: 13, textDecoration: 'line-through', color: '#aaa' }}>{product.sellingPrice} ج.م</span>}
          </div>
          <button onClick={handleAdd} disabled={product.stock < 1} style={{ width: '100%', padding: '9px', background: product.stock < 1 ? '#ccc' : '#1a3a5c', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 14, cursor: product.stock < 1 ? 'not-allowed' : 'pointer' }}>
