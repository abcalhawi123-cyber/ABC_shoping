...
export default function MyOrders() {
  ...
  return (
    <>
      <Helmet><title>{t('myOrders')} — ABC الحاوي</title></Helmet>
      <h1 style={{ color: '#1a3a5c', marginBottom: 20, fontSize: 22 }}>📦 {t('myOrders')}</h1>
      ...
      {loading ? (
        ...
        <>
          {orders.map(order => {
            ...
            return (
              <div key={order._id} style={{ background: '#fff', borderRadius: 12, padding: '16px', marginBottom: 14, boxShadow: '0 2px 10px rgba(0,0,0,0.06)', border: '1px solid #f0e8e8' }}>
                {/* Header row */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
                  <div>
                    <p style={{ margin: '0 0 3px', fontSize: 11, color: '#bbb', fontFamily: 'monospace' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                    <span style={{ background: s.bg, color: s.c, padding: '4px 10px', borderRadius: 12, fontSize: 12, fontWeight: 600 }}>
                      {order.status}
                    </span>
                    <span style={{ fontWeight: 700, color: '#1a3a5c', fontSize: 16 }}>{order.total} ج.م</span>
                  </div>
                </div>
                ...
                <div style={{ borderTop: '1px solid #f5f5f5', paddingTop: 10, marginBottom: 12 }}>
                  {order.items?.map((item, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 5, color: '#555' }}>
                      <span>{item.name?.[lang] || item.name?.ar} × {item.quantity}</span>
                      <span style={{ fontWeight: 600, flexShrink: 0 }}>{(item.unitPrice * item.quantity).toFixed(0)} ج.م</span>
                    </div>
                  ))}
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', borderTop: '1px solid #f5f5f5', paddingTop: 10, alignItems: 'center' }}>
                  ...
                  {eligible && (
                    <button onClick={() => requestReturn(order._id)} style={{ padding: '7px 14px', background: '#fff3e0', color: '#ef6c00', border: '1px solid #ffcc80', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                      🔄 {t('requestReturn')}
                    </button>
                  )}

                  {order.status === 'تم التسليم' && order.isReturnEligible && (
                    <span style={{ fontSize: 11, color: eligible ? '#27ae60' : '#e74c3c' }}>
                      {eligible
