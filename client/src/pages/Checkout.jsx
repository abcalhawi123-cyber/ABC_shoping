...
export default function Checkout() {
  ...
  useEffect(() => {
    getShippingZones().then(r => setZones(r.data.data)).catch(() => {});
  }, []);
  ...
  const submit = async e => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || (lang === 'ar' ? 'حدث خطأ، حاول مرة أخرى' : 'An error occurred'));
    }
    setBusy(false);
  };
  ...
  return (
    <>
      <Helmet><title>{t('checkout')} — ABC {lang === 'ar' ? 'الحاوي' : 'Al-Hawi'}</title></Helmet>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 32, alignItems: 'start' }}>
        <form onSubmit={submit}>
          ...
          <div style={sec}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              >
                ...
                {zones.map(z => (
                  <option key={z._id} value={z.governorate.ar}>
                    {lang === 'ar' ? z.governorate.ar : z.governorate.en}
                  </option>
                ))}
              </select>
              {zone && (
          ...
          <div style={sec}>
            ...
            {pay === 'instapay' && (
              <div style={{ background: '#f0f8ff', borderRadius: 10, padding: 16, marginTop: 8, border: '1px solid #bbdefb' }}>
            ...
            {pay === 'card' && (
              <div style={{ background: '#f0f8ff', borderRadius: 10, padding: 16, marginTop: 8 }}>
                <p style={{ fontSize: 14, color: '#555', margin: 0 }}>
                  {lang === 'ar' ? 'ستنتقل إلى بوابة Paymob الآمنة لإتمام الدفع.' : 'You will be redirected to Paymob secure payment gateway.'}
                </p>
              </div>
            )}
          </div>
          ...
          <button
            type="submit"
            disabled={busy}
            style={{ width: '100%', padding: '16px', background: busy ? '#aaa' : '#1a3a5c', color: '#fff', border: 'none', borderRadius: 10, fontSize: 17, fontWeight: 700, cursor: busy ? 'not-allowed' : 'pointer' }}
          >
        ...
        <div style={{ position: 'sticky', top: 90 }}>
          <div style={sec}>
            ...
            {cart?.map(item => {
              ...
              return (
                <div key={item._id} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10, fontSize: 14 }}>
                  <span style={{ flex: 1, color: '#555' }}>{name} × {item.qty}</span>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <span style={{ color: zone ? '#27ae60' : '#888' }}>
                {zone ? `${zone.price} ج.م` : '—'}
              </span>
            </div>
...
const secT = { margin: '0 0 16px', color: '#1a3a5c', fontSize: 17, fontWeight: 700 };
