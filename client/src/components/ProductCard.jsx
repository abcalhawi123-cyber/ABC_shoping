...
export default function ProductCard({ product }) {
  ...
  return (
    <Link to={`/products/${product.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
      <div style={{ background: '#fff', borderRadius: 14, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.07)', border: '1px solid #f0e8e8' }}
        ...
        <div style={{ position: 'relative', aspectRatio: '1', background: '#faf5f5' }}>
          ...
          {product.discount > 0 && <span style={{ position: 'absolute', top: 10, right: 10, background: '#f8ad9d', color: '#1a3a5c', padding: '3px 8px', borderRadius: 12, fontSize: 12, fontWeight: 700 }}>-{product.discount}%</span>}
          {product.stock < 1 && <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ color: '#fff', fontWeight: 700 }}>{t('outOfStock')}</span></div>}
          {product.stock > 0 && product.stock < 5 && <span style={{ position: 'absolute', bottom: 8, right: 8, background: '#e74c3c', color: '#fff', padding: '2px 7px', borderRadius: 10, fontSize: 11 }}>{product.stock} {lang === 'ar' ? 'قطع فقط!' : 'left!'}</span>}
        </div>
        <div style={{ padding: '12px 14px' }}>
          ...
          {product.averageRating > 0 && <div style={{ fontSize: 12, color: '#f39c12', marginBottom: 6 }}>{'★'.repeat(Math.round(product.averageRating))}{'☆'.repeat(5 - Math.round(product.averageRating))} <span style={{ color: '#888' }}>({product.reviewCount})</span></div>}
